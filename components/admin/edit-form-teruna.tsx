...
import { UpdateTeruna } from "@/lib/action";
import { TerunaProps } from "@/types/teruna";

const EditFormTeruna = ({ teruna }: { teruna: TerunaProps }) => {
  const [sektorList, setSektorList] = useState<string[]>([]);
  const [selectedSektor, setSelectedSektor] = useState<string>(teruna.sektor);
  const [openSektor, setOpenSektor] = useState(false);
  const [genderList, setGenderList] = useState<string[]>([]);
  const [selectedGender, setSelectedGender] = useState<string>(teruna.gender);
  const [openGender, setOpenGender] = useState(false);
  const [openCalendar, setOpenCalendar] = useState(false);
  const [date, setDate] = useState<Date | undefined>(
    teruna.birth ? new Date(teruna.birth) : undefined,
  );
  useEffect(() => {
  ...
  const [state, formAction, isPending] = useActionState(
    UpdateTeruna.bind(null, teruna.id),
    null,
  );
    ...
    <form action={formAction}>
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2">
          <Label>Name</Label>
          <Input name="name" defaultValue={teruna.name} placeholder="Stephen" />
